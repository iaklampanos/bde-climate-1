...
run-wps::
	...
	if [ "$$CRES" = "" ];then \
	  CUUID=`uuidgen`;\
	  /usr/bin/docker exec -it bdeclimate1_cassandra_1 cqlsh -e "INSERT INTO testprov.prov (id, isvalid, paths, paramset, type, downscaling, createdat, lasteditedat) VALUES ($$CUUID, False, {'/home/bde2020/data/BINARY/'}, {'sst:$(RSTARTDT)','wps:1','wrf:0','d01:$$d01','d02:$$d02','d03:$$d03','d01rd:$(RDURATION)','d02rd:$(RDURATION)','d03rd:$(RDURATION)','d01k:$$d01','d02k:$$d02','d03k:$$d03'}, 'wps',[{agentname: 'wps', agenttype:'software', agentversion:'0.0.1', st:toTimestamp(now()), et:toTimestamp(now()), params:{'d01':'$$d01','d02':'$$d02','d03':'$$d03'}, issuccessful:False}], toTimestamp(now()), toTimestamp(now()))";\
	  ...
	  /usr/bin/docker exec -it bdeclimate1_cassandra_1 cqlsh -e "UPDATE testprov.prov set isvalid=True, paths={$$ipaths}, lasteditedat=toTimestamp(now()),downscaling=[$$ALISTI] +downscaling WHERE id=$$CUUID";\
	  echo "PROV_ID_WPS_ "$$CUUID;\
	else\
	  CPAR=`/usr/bin/docker exec -it bdeclimate1_cassandra_1 cqlsh -e " select id from testprov.prov where isvalid=True and paramset contains 'wps:1' and paramset contains 'sst:$(RSTARTDT)' and paramset contains 'd01:$$d01' and paramset contains 'd02:$$d02' and paramset contains 'd03:$$d03' and paramset contains 'd01rd:$(RDURATION)' and paramset contains 'd02rd:$(RDURATION)' and paramset contains 'd03rd:$(RDURATION)' and paramset contains 'd01k:$$d01' and paramset contains 'd02k:$$d02' and paramset contains 'd03k:$$d03' limit 1 allow filtering"| tail -n3 |head -n1 |sed 's| ||g'`;\
	  echo "PROV_ID_WPS_ "$$CPAR;\
	  nclist=`/usr/bin/docker exec -it bdeclimate1_cassandra_1 cqlsh -e "select paths from testprov.prov where id=$$CPAR" | head -n4 | tail -n1 | tr -d "',{,}"`;\
	  ...
	  scp ./out_met_em.$(REG)* $(USERNAM)@$(MODELSRV):~/$$CURRUUID/Run/WPS/RunData/;\
	fi;\
	make run-ssh-cp;\
	rm -f ./met_em.$(REG)*;
...
run-wrf-nest::
	...
	if [ "$(REG)" = d02d03 ];then \
	 d03=12;\
	 reg=d03;\
	 PFWRF=`make -s run-wrf REG=d02 | grep "PROV_ID_WRF_"`;\
	 PFWPS=`make -s run-wps REG=d03 | grep "PROV_ID_"`;\
	 PFWRFI=`echo $$PFWRF | awk -F " " '{print $$2}'`;\
	 PFWPSI=`echo $$PFWPS | awk -F " " '{print $$2}'`;\
	fi;\
	CRES=`/usr/bin/docker exec -it bdeclimate1_cassandra_1 cqlsh -e " select id from testprov.prov where "\
	" bparentid=$$PFWPSI and parentid=$$PFWRFI and isvalid=True and paramset contains 'wrf:1' and paramset contains 'sst:$(RSTARTDT)' and paramset contains 'd01:$$d01'"\
	" and paramset contains 'd02:$$d02' and paramset contains 'd03:$$d03' and paramset contains 'd01rd:$(RDURATION)' "\
	" and paramset contains 'd02rd:$(RDURATION)' and paramset contains 'd03rd:$(RDURATION)' and paramset contains 'd01k:$$d01'"\
	" and paramset contains 'd02k:$$d02' and paramset contains 'd03k:$$d03' limit 1 allow filtering"| tail -n1| grep 1`;\
	if [ "$$CRES" = "" ]; then \
	...
	"INSERT INTO testprov.prov (id, isvalid, parentid, bparentid, paths, paramset, type, downscaling, createdat, lasteditedat) "\
	"VALUES ($$CUUID, False, $$PFWRFI,$$PFWPSI, {'/home/bde2020/data/BINARY/'}, "\
...
run-wrf::
	...
	for f in wrfout_d01*; do \
	 ...
	 make ingest-file NETCDFFILE=$$ftc;\
	done;\
	ALISTI=`/usr/bin/docker exec -it bdeclimate1_cassandra_1 cqlsh -e "select downscaling from testprov.prov where id=$$CUUID;"`;\
	/usr/bin/docker exec -it bdeclimate1_cassandra_1 cqlsh -e "delete downscaling from testprov.prov where id=$$CUUID";\
	ALISTI=`echo $$ALISTI|sed 's|.*\[||;s|].*||;s|000+0000||g;s|issuccessful: False|issuccessful: True|' `;\
