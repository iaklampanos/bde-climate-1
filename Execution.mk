...
run-wps::
	...
	if make -f sc5_query.mk PROV_SEL_ \
	  TYPE=wps CUSER=$(CUSER) RDURATION=$(RDURATION) RSTARTDT=$(RSTARTDT) &> cql_log; then\
	  CRES=`make -f sc5_query.mk PROV_SEL_ \
	  TYPE=wps CUSER=$(CUSER) RDURATION=$(RDURATION) RSTARTDT=$(RSTARTDT) | tail -n1 | grep 1`;\
	else\
	...
	if [ "$$CRES" = "" ];then\
	  ...
	  if make -f sc5_query.mk PROV_INS_ TYPE=wps\
	     CUUID=$$CUUID CUSER=$(CUSER) RDURATION=$(RDURATION) RSTARTDT=$(RSTARTDT) &> cql_log; then\
	  ...
	  echo "PROV_ID_WPS_ "$$CUUID;\
	    ...
	    TYPE=wps CUSER=$(CUSER) RDURATION=$(RDURATION) RSTARTDT=$(RSTARTDT) | tail -n3 |head -n1 |sed 's| ||g'`;\
...
run-wrf::
	...
	if [ "$(REG)" = "d03" ]; then d03=1; fi;\
	CURRUUID=`cat $(CUSER)_curr.UUID`;\
	make $(MAKEOPTS) -s run-wps | tee CURRUUID_log &&\
	PF=`cat CURRUUID_log | grep "PROV_ID_WPS_"`;\
	PFI=`echo $$PF | awk -F " " '{print $$2}'`;\
	if make -f sc5_query.mk PROV_SEL_ TYPE=wrf \
	  CUUID=$$CUUID CUSER=$(CUSER) RDURATION=$(RDURATION) RSTARTDT=$(RSTARTDT) &> cql_log; then\
	  CRES=`make -f sc5_query.mk PROV_SEL_ \
	  TYPE=wrf CUSER=$(CUSER) RDURATION=$(RDURATION) RSTARTDT=$(RSTARTDT) | tail -n1 | grep 1`;\
	else\
	  echo "Progress: Error in CQL command, check cql_log file";\
	  exit 1;\
	fi;\
	if [ "$$CRES" = "" ];then\
	  ...
	  CPAR=`make -s -f sc5_query.mk PROV_SEL_ \
	  TYPE=wrf CUSER=$(CUSER) RDURATION=$(RDURATION) RSTARTDT=$(RSTARTDT) | tail -n3 |head -n1 |sed 's| ||g'`;\
...
run-wrf-nest::
	...
	if [ "$(REG)" = d01d02 ];then \
	  d02=12;\
	  reg=d01;\
	  make $(MAKEOPTS) -s run-wrf REG=$$reg | tee CURRUUID_log &&\
	  PFWRF=`cat CURRUUID_log | grep "PROV_ID_WRF_"`;\
	  reg=d02;\
	  make $(MAKEOPTS) -s run-wps REG=$$reg | tee CURRUUID_log &&\
	...
	if make -f sc5_query.mk PROV_SEL_NEST_ PWRFID=$$PFWRFI BPWRFID==$$PFWPSI \
	  CUSER=$(CUSER) RDURATION=$(RDURATION) RSTARTDT=$(RSTARTDT) &> cql_log; then\
	  CRES=`make -f sc5_query.mk PROV_SEL_NEST_ PWRFID=$$PFWRFI BPWRFID==$$PFWPSI \
	  CUSER=$(CUSER) RDURATION=$(RDURATION) RSTARTDT=$(RSTARTDT)| tail -n1 | grep 1`;\
	else\
	  echo "Progress: Error in CQL command, check cql_log file";\
	  exit 1;\
	fi;\
