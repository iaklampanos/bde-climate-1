...
PROV_WPS_SEL_::
	...
	/usr/bin/docker exec -i bdeclimate1_cassandra_1 cqlsh -e "SELECT id from testprov.prov where isvalid=True and user='$(CUSER)' and paramset contains 'wps:1' and paramset contains 'sst:$(RSTARTDT)' and paramset contains 'd01:$$d01' and paramset contains 'd02:$$d02' and paramset contains 'd03:$$d03' and paramset contains 'd01rd:$(RDURATION)' and paramset contains 'd02rd:$(RDURATION)' and paramset contains 'd03rd:$(RDURATION)' and paramset contains 'd01k:$$d01' and paramset contains 'd02k:$$d02' and paramset contains 'd03k:$$d03' limit 1 allow filtering";
...
PROV_WPS_INS_::
	d01=0;d02=0;d03=0;reg=$(REG);\
        if [ "$(REG)" = "d01" ]; then d01=1; fi;\
        if [ "$(REG)" = "d02" ]; then d02=1; fi;\
        if [ "$(REG)" = "d03" ]; then d03=1; fi;echo $(REG);echo $$d01;\
	/usr/bin/docker exec -i bdeclimate1_cassandra_1 cqlsh -e "INSERT INTO testprov.prov (id, user, isvalid, paths, paramset, type, downscaling, createdat, lasteditedat) VALUES ($(CUUID), '$(CUSER)', False, {'/home/bde2020/data/BINARY/'}, {'sst:$(RSTARTDT)','wps:1','wrf:0','d01:$$d01','d02:$$d02','d03:$$d03','d01rd:$(RDURATION)','d02rd:$(RDURATION)','d03rd:$(RDURATION)','d01k:$$d01','d02k:$$d02','d03k:$$d03'}, 'wps',[{agentname: 'wps', agenttype:'software', agentversion:'0.0.1', st:toTimestamp(now()), et:toTimestamp(now()), params:{'d01':'$$d01','d02':'$$d02','d03':'$$d03'}, issuccessful:False}], toTimestamp(now()), toTimestamp(now()))";


PROV_UPD_::
	AL=`echo $(ALISTI) | sed 's|_pb_|(|g;s|_pe_|)|g'`; path=`echo "{"$(ipaths)"}"`; echo $$path;\
	echo "UPDATE testprov.prov set isvalid=True, paths=$(ipaths), lasteditedat=toTimestamp(now()),downscaling=[$$AL] +downscaling WHERE id=$(CUUID)";\
